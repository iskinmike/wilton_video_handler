...
recognizer::recognizer(recognizer_settings settings):
...
void recognizer::setup_frame_keeper(std::shared_ptr<frame_keeper> keeper){
...
matcher::matcher(matcher_settings set): set(set) {}
...
template_detector::template_detector(const cv::Mat &image, const cv::Rect &face, template_detector_settings set):
    is_face_detected(false), set(set) {
