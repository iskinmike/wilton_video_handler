...
void background_texture::setup_shader_program(){
    shader = shader_class{background_texture::vertex_shader_text, background_texture::fragment_shader_text};
    shader.compile_shaders();
    program = shader.create_program();
    vertex_texture_location = glGetUniformLocation(program, "loaded_texture");
}

void background_texture::bind_EBO(){
    glGenBuffers(1, &EBO);
...
void background_texture::bind_VBO(){
...
void background_texture::load_texture_rgba(int size_x, int size_y, uint8_t *data){
...
void background_texture::draw_background(){
    shader.use_program();
    glBindTexture(GL_TEXTURE_2D, texture);
...
std::string background_texture::fragment_shader_text =
"#version 330 core\n"
"in vec2 tex_coords;"
"out vec4 color;"
...
"}\n";
std::string background_texture::vertex_shader_text =
"#version 330 core\n"
...
GLfloat background_texture::vertices[20] = {
...
GLuint background_texture::indices[6] = {  // Помните, что мы начинаем с 0!
    0, 1, 3,   // Первый треугольник
    1, 2, 3    // Второй треугольник
};

GLuint background_texture::get_VAO() const
{
    return VAO;
}

GLuint background_texture::get_texture() const
{
    return texture;
}
